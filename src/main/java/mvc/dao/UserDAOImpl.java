...
public class UserDAOImpl implements UserDAO {
    ...
    public UserPage readPage(PageStateInfo pageStateInfo) {
        ...
        //
        //query.setParameter("fld", fld.isEmpty() ? "id" : fld); //похоже, в ORDER BY параметры не катят
    ...
    public Integer delete(long id) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("DELETE User WHERE id = :id"); // HSL - удобно. Есть другие варианты - попробовать
